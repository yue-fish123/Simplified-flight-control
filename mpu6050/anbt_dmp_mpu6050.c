...
/*
初始化调用  AnBT_DMP_MPU6050_Init();
任务中调用  AnBT_DMP_MPU6050_SEND_DATA_FUN(); 每秒最多执行 1180次（M4@168MHz）
...
*/
unsigned char AnBT_DMP_I2C_Write(unsigned char anbt_dev_addr, unsigned char anbt_reg_addr, unsigned char anbt_i2c_len, unsigned char *anbt_i2c_data_buf)
{		
	return HAL_I2C_Mem_Write(&hi2c2,anbt_dev_addr,anbt_reg_addr,1,anbt_i2c_data_buf,anbt_i2c_len,1000);
	//return 0;
}
unsigned char AnBT_DMP_I2C_Read(unsigned char anbt_dev_addr, unsigned char anbt_reg_addr, unsigned char anbt_i2c_len, unsigned char *anbt_i2c_data_buf)
{
	return HAL_I2C_Mem_Read(&hi2c2,anbt_dev_addr,anbt_reg_addr,1,anbt_i2c_data_buf,anbt_i2c_len,1000);
	//return 0;
}
...
const struct gyro_reg_s reg= {	 //const是一个C语言的关键字，它限定一个变量不允许被改变。使用const在一定程度上可以提高程序的安全性和可靠性。
0x75,  //who_am_i
0x19,  //rate_div
0x1A,  //lpf
0x0C,  //prod_id
...
//
int mpu_set_gyro_fsr(unsigned short fsr)  //FSR=Feedback Shift Register 反馈移位寄存器
                                          //或者FSR=Full Scale Range 满刻度、满量程
                                          //用来与INDF寄存器配合使用,实现寄存器间接寻址,也就是用INDF来访问FSR内容为地址的RAM中的内容.
{
    unsigned char data;
...
unsigned char AnBT_DMP_MPU6050_DEV_CFG(void)		
{
		...
		//MPU6050储存ID的寄存器即0x75，ID是默认值0x68
		AnBT_DMP_I2C_Read(st.hw->addr, st.reg->who_am_i, 1, &(anbt_dmp_data[0]));
			 
	  printf("ADDR = %02x\r\n",anbt_dmp_data[0]);
		if (anbt_dmp_data[0]!=PRODUCT_WHOAMI) 
		{
			printf("I2C从机地址设置错误，请将AD0端口接地\r\n");
			printf("ADDR = %02x\r\n",anbt_dmp_data[0]);
			return 2;    
    }
		/*
		st.reg->pwr_mgmt_1
		电源管理寄存器1：0x6B    bit7： 置1时MPU6050复位，成功后自动清0
		*/
    ...
    AnBT_DMP_Delay_ms(100);
		//
    anbt_dmp_data[0] = 0x00; //etootle: Wake up chip			唤醒  正常启动 内部20M晶振  正常工作模式  使能温度传感器    配置为0x01。自动选择最好的时钟源
    if (AnBT_DMP_I2C_Write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(anbt_dmp_data[0])))
			return 4;
    /*
		st.reg->accel_offs
...
{
	uint8_t dmp_init_state = AnBT_DMP_MPU6050_DEV_CFG();		//etootle: init the mpu6050 chip				
	if(dmp_init_state == 0){
		printf("1.DMP设备初始化成功"); 		//完成初始化
		//HAL_GPIO_WritePin(LED3_GPIO_Port,LED3_Pin,1);
	}else{
		printf("1.DMP设备初始化失败 %u\r\n",dmp_init_state); 		//完成初始化
		//HAL_GPIO_WritePin(LED1_GPIO_Port,LED1_Pin,1);
	}
	
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("2.DMP设置传感器成功");		//完成传感去设定
	else printf("2.DMP设置传感器错误");
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("3.设置FIFO成功");		//完成fifo设置
	else printf("3.设置FIFO失败");
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))printf("4.DMP设置采样率成功");			//完成DMP采样率设置
	else printf("4.DMP设置采样率失败");
	//
	if(!dmp_load_motion_driver_firmware())	printf("5.DMP加载运动驱动固件成功");				//开启DMP功能
	else printf("5.DMP加载运动驱动固件失败");
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) printf("6.DMP完成设置初始方向设定偏差");		//初始方向设定偏差
	else printf("6.DMP设置初始方向设定偏差出错");
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))		printf("7.DMP使能成功");		//DMP使能
	else printf("7.DMP使能出错");
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) printf("8.DMP设置FIFO速率成功");		//DMP  fifo速度
	else printf("8.DMP设置FIFO速率失败");
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)) printf("9.Dmp启动成功");				//DMP成功开启
	else printf("9.Dmp启动失败");					//Dmp启动失败
	//
	return 0;
}
...
*/
void ANO_DT_Send_Status(float angle_rol, float angle_pit, float angle_yaw, int32_t alt, unsigned char fly_model, unsigned char armed)
{
  ...
  int16_t _temp;
    ...
    data_to_send[_cnt++]=0xAF;//上位机  此处必须这样填
    data_to_send[_cnt++]=0x01;//匿名四轴  此处必须这样填
    data_to_send[_cnt++]=0x01;//姿态信息
		data_to_send[_cnt++]=0x00;//数据长度
    ...
    data_to_send[_cnt++]=alt >> 24;
    data_to_send[_cnt++]=alt >> 16;
    data_to_send[_cnt++]=alt >> 8;
    data_to_send[_cnt++]=alt;
    ...
    //UART_Send_Data(&huart1,data_to_send,_cnt);
		
		//Print_Array(data_to_send,_cnt);
}
...
void MPU_ReadTemperature(float *temperature)
{
	...
	AnBT_DMP_I2C_Read(st.hw->addr, st.reg->temp, 2, buf);
...
{
//	float Yaw = 0;
	float Roll_Exchange = 0;
//	float Pitch = 0;//欧拉角
	float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;//计算姿态过程用到的变量
	float q30 = 1073741824.0f;
	int8_t get = dmp_read_fifo(gyro, accel, anbt_mpu6050_quat_data, &sensor_timestamp, &sensors, &more);	 
	if(get != 0){
		return;
	}
	if(sensors&INV_WXYZ_QUAT){	 
		 q0=anbt_mpu6050_quat_data[0] / q30;
		 q1=anbt_mpu6050_quat_data[1] / q30;
		 q2=anbt_mpu6050_quat_data[2] / q30;
		 q3=anbt_mpu6050_quat_data[3] / q30;
		 Pitch_Angle_C = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3; // Pitch_Angle_C
		 Roll_Exchange = -atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3; // Roll_Angle_C*-1
		 Yaw_Angle_C = 	-atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//Yaw_Angle_C*-1
		//printf("q0 = %f,q1 = %f,q2 = %f,q3 = %f\r\n",q0,q1,q2,q3);
		if(Roll_Exchange < 0){
			Roll_Angle_C = (Roll_Exchange+180);
		}else if(Roll_Exchange > 0){
			Roll_Angle_C = Roll_Exchange-180;
		}
		...
		//ANO_DT_Send_Status(Roll,Pitch,Yaw,100,1,1);
	 }else{
		printf("sensors&INV_WXYZ_QUAT ERROR\r\n");
	 }
	
	 
	 //float t = 0;
	 //MPU_ReadTemperature(&t);
	 //printf("Temp = %.2f\r\n",t);
...
void AnBT_DMP_Delay_ms(unsigned int dly){
	HAL_Delay(dly);
}
